...
public:
    ...
    {
        r = MIN(fMax, MAX(fMin, r));
        g = MIN(fMax, MAX(fMin, g));
        b = MIN(fMax, MAX(fMin, b));
        a = MIN(fMax, MAX(fMin, a));
        return *this;
    }
...
struct D3DX_BC1
{
    unsigned short int    rgb[2]; // 565 colors
    unsigned int		  bitmap; // 2bpp rgb bitmap
};

// BC2/DXT2/3 compression (8 bits per texel)
struct D3DX_BC2
{
    unsigned int    bitmap[2];  // 4bpp alpha bitmap
...
struct D3DX_BC3
{
    unsigned char   alpha[2];   // alpha values
    unsigned char   bitmap[6];  // 3bpp alpha bitmap
    D3DX_BC1		bc1;        // BC1 rgb data
};
#pragma pack(pop)
...
template <bool bRange> void OptimizeAlpha(float *pX, float *pY, const float *pPoints, unsigned int cSteps)
{
...
//////////////////////////////////////////////////////////////////////////////////////////////////
#define FLT_MIN 0.000000f
#define FLT_MAX 340282346638528859811704183484516925440.000000f
